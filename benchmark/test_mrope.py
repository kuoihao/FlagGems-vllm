...
class MropeBenchmark(base.Benchmark):
    def set_shapes(self, shape_file_path=None):
        self.shapes = ALL_CONFIGS[:]
        self.shape_desc = (
            "num_tokens, q_heads, kv_heads, head_size, rotary_dim, st, sh, sw, "
            "neox, interleaved, glm"
        )
    ...
    def get_input_iter(self, dtype):
        for seed, config in enumerate(self.shapes):
            (
                num_tokens,
                num_q_heads,
                num_kv_heads,
            ...
            positions = torch.randint(
                0, MAX_POSITION, (3, num_tokens), device=self.device
            )
            query = torch.randn(
                num_tokens,
            ...
            cos_sin_cache = _compute_cos_sin_cache(
                MAX_POSITION, rotary_dim, BASE, torch.bfloat16
            ).to(self.device)
            axis_map = None
            if is_interleaved_glm:
                axis_map = _compute_axis_map(
                    [section_t, section_h, section_w], rotary_dim
                ).to(self.device)
...
@pytest.mark.mrope
def test_mrope_gems_vs_torch():
    from flaggems_vllm.ops.mrope import mrope as gems_mrope

    bench = MropeBenchmark(
        op_name="mrope",
        torch_op=torch_mrope,
        dtypes=[torch.bfloat16],
    )
    bench.set_gems(gems_mrope)
    bench.run()
