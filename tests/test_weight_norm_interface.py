...
def test_weight_norm_interface(shape, dtype, dim):
    ...
    dim = dim % len(shape)
    v = torch.randn(shape, dtype=dtype, device=flaggems_vllm.device)
    g = torch.randn(shape[dim], dtype=dtype, device=flaggems_vllm.device)
...
def test_weight_norm_interface_backward(shape, dtype, dim):
    ...
    res_g = torch.randn(shape[dim], dtype=dtype, device=flaggems_vllm.device)
