...
def _build_inputs(config):
    ...
    positions = torch.randint(
        0, MAX_POSITION, (3, num_tokens), device=flaggems_vllm.device
    )
    query = torch.randn(
        num_tokens, num_q_heads * head_size, device=flaggems_vllm.device, dtype=dtype
    )
    key = torch.randn(
        num_tokens, num_kv_heads * head_size, device=flaggems_vllm.device, dtype=dtype
    )
